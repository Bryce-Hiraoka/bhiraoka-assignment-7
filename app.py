...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y =     Y = beta0 + beta1 * X + mu + error

    # TODO 3: Fit a linear regression model to X and Ye
...
def hypothesis_test():
    ...
    fun_message = None
    if p_value <= 0.0001:
        fun_message = "Whoa! You've discovered a rare event! This result is very unlikely under the null hypothesis."

    # TODO 12: Plot histogram of simulated statistics
    plot3_path = "static/plot3.png"
    plt.figure(figsize=(10, 5))
    plt.hist(simulated_stats, bins=20, alpha=0.7, color="purple", label="Simulated Statistics")
    plt.axvline(observed_stat, color="green", linestyle="--", linewidth=2, label=f"Observed: {observed_stat:.2f}")
    plt.axvline(hypothesized_value, color="red", linestyle="--", linewidth=2, label=f"Hypothesized: {hypothesized_value:.2f}")
